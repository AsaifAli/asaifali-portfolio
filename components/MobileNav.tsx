...
type Props = { nav: string[][] };

export function MobileNav({ nav }: Props) {
  const [open, setOpen] = useState(false);
  return (
    <div className="mobile-nav">
      <button type="button" className="mobile-nav-toggle" aria-expanded={open} aria-controls="mobile-nav-panel" onClick={() => setOpen(v => !v)}>
        <span>{open ? "Close" : "Menu"}</span><span className="mobile-nav-icon" aria-hidden="true">{open ? "×" : "☰"}</span>
      </button>
      {open && (
