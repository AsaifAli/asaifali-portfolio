...
export default async function Home() {
  ...
  return <main>
    <section className="shell hero">
      ...
      <div className="hero-meta"><Link className="btn btn-primary" href="#work">Explore projects ↓</Link><Link className="btn btn-secondary" href="#demo">Try live demo →</Link><a className="btn btn-secondary" href="https://github.com/AsaifAli" target="_blank" rel="noreferrer">GitHub ↗</a></div>
      <div className="hero-grid"><div className="signal-card"><div className="signal-top"><span>system profile</span><span>online</span></div><div className="signal-lines">{[1,1,1,1,1,1,0,1].map((on,i)=><span className={on?"on":""} key={i} />)}</div><p className="muted">Probabilistic intelligence is paired with deterministic controls — typed outputs, evidence checks, stateful workflows, quality gates, and regression tests.</p></div><div className="manifest"><code>{`{
  "focus": [
    "Agentic AI",
    "RAG",
    "LLM Systems",
  ...
  "default": "engineered, not improvised"
    ...
    <section id="work" className="shell section"><div className="section-head"><div><div className="section-kicker">Selected work</div><h2>Systems I built to be trusted.</h2></div><p className="section-intro">Five portfolio projects across modernization, evidence, procurement, automation, and browser intelligence.</p></div><div className="project-list">{projects.map(p=><ProjectCard key={p.slug} project={p}/>)}</div></section>

    <section id="demo" className="shell section"><div className="section-head"><div><div className="section-kicker">Interactive demo · BYOK</div><h2>Try the shared inference boundary.</h2></div><p className="section-intro">Bring your own provider key to create a temporary session. The portfolio never stores the provider credential.</p></div><InteractiveDemo project="portfolio" title="Try the interactive demo" description="Use your own provider key to open a short-lived inference session. The credential is used only for the session request." /></section>
    ...
    <section id="contact" className="shell section"><div className="contact-card"><div><div className="section-kicker">Contact</div><h2>Have an AI system worth building?</h2><p>For interviews and technical discussions, the portfolio links directly to the code and live deployments.</p></div><div><ContactForm/></div></div></section>
  </main>;
}
